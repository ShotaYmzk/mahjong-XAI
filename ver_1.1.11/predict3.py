...
def explain_prediction_with_shap(model, background_data_path, instance_to_explain, feature_names, target_class_index, n_shap_samples=100, n_bg_summary_samples=50):
    ...
    if not bg_sequences_list or not bg_static_features_list:
...
if __name__ == "__main__":
    ...
    try:
        ...
        model = load_trained_model_for_prediction(args.model_path, event_dim, static_dim, args.visualize_attention)

        logging.info("打牌を予測中...")
        predicted_index, predicted_prob, all_probabilities, collected_attention_weights = predict_discard(
            model, game_state, player_id, visualize_attention_flag=args.visualize_attention
        )
        ...
        for i in range(min(top_n, len(indices_sorted))): # 修正2: min を使用
            ...
            if 0 <= idx < NUM_TILE_TYPES:
                ...
                print(f"  {i+1}. {tile_str_top:<4s} ({prob:.4f})")
        ...
        if shap_available:
            try:
                ...
                _ = explain_prediction_with_shap(model, DATA_HDF5_PATH, instance_to_explain, feature_names, predicted_index, args.shap_samples, args.background_samples)
            except Exception as shap_e: logging.error(f"\n[エラー] SHAP説明の生成中にエラー: {shap_e}", exc_info=True)
