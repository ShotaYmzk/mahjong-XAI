...
MAX_FILES_TO_PROCESS = 30000  # 処理するXMLファイルの最大数（ランダムサンプリング用）
RANDOM_SEED = 42  # ランダムサンプリングのシード値
...
def main():
    ...
    # 全XMLファイルを取得し、ランダムにサンプリング
    all_xml_files = sorted(glob.glob(os.path.join(XML_LOG_DIR, "*.xml")))
    total_available_files = len(all_xml_files)
    ...
    if total_available_files > MAX_FILES_TO_PROCESS:
        xml_files = random.sample(all_xml_files, MAX_FILES_TO_PROCESS)
        logging.info(f"Randomly selected {MAX_FILES_TO_PROCESS} files from {total_available_files} available XML files.")
    else:
        xml_files = all_xml_files
        logging.info(f"Using all {total_available_files} available XML files (less than MAX_FILES_TO_PROCESS={MAX_FILES_TO_PROCESS}).")

    total_files = len(xml_files)
    logging.info(f"Processing {total_files} XML files from {XML_LOG_DIR}")
