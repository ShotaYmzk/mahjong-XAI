...
def train_model():
    ...
    if train_size <= 0 or val_size <= 0:
         logging.error(f"Invalid dataset split: Train={train_size}, Val={val_size}, Test={test_size}. Check dataset size ({total_samples}) and split ratios (Train={TRAINING_SPLIT}, Val={VALIDATION_SPLIT}). Aborting training.")
         return
    ...
    test_dataset = Subset(full_dataset, test_indices) if test_size > 0 else None

    logging.info(f"Dataset split: Train samples={len(train_dataset)}, Validation samples={len(val_dataset)}, Test samples={len(test_dataset) if test_dataset else 0}")
