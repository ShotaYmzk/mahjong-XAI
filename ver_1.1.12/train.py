...
from torch.optim.lr_scheduler import CosineAnnealingLR # 学習率スケジューラ
...
# --- データ関連 ---
# HDF5 ファイルのパスを指定
DATA_HDF5_PATH = "/home/ubuntu/Documents/mahjong-XAI/ver_1.1.12/training_data/mahjong_imitation_data_v1112.hdf5" 
VALIDATION_SPLIT = 0.05                                 # データセットからバリデーション用に分割する割合

# --- モデル保存関連 ---
MODEL_SAVE_PATH = "./trained_model/mahjong_transformer_v1112_large_compiled.pth" # 最良モデルの保存パス (compile版を示す名前に変更)
CHECKPOINT_DIR = "./checkpoints_v1112_large_compiled/"       # チェックポイントの保存ディレクトリ (compile版を示す名前に変更)
...
LEARNING_RATE = 1e-4        # 学習率の初期値
WEIGHT_DECAY = 0.01         # AdamWのWeight Decay (正則化)
...
DROPOUT = 0.05               # ドロップアウト率（0.05に下げる）
...
def analyze_dataset_distribution(train_loader, val_loader):
    train_labels = []
    val_labels = []
    for _, _, labels, _ in train_loader:
        train_labels.extend(labels.numpy())
    for _, _, labels, _ in val_loader:
...
# =                            Main Training Loop                            =
# ==============================================================================
def load_checkpoint(checkpoint_path, model, optimizer=None, scheduler=None, scaler=None, ema=None):
    ...
    try:
        ...
        # オプティマイザの状態を読み込む（指定されている場合）
        if optimizer is not None and 'optimizer_state_dict' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        
        # スケジューラの状態を読み込む（指定されている場合）
        if scheduler is not None and 'scheduler_state_dict' in checkpoint:
            scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
        
        # スケーラーの状態を読み込む（指定されている場合）
        if scaler is not None and 'scaler_state_dict' in checkpoint:
            scaler.load_state_dict(checkpoint['scaler_state_dict'])
        
        # EMAの状態を読み込む（指定されている場合）
...
def train_model(resume_training=False):
    ...
    # --- データセット分布の分析（エポック0で1回だけ） ---
    analyze_dataset_distribution(train_loader, val_loader)
    ...
    logging.info(f"Using LR Scheduler: CosineAnnealingLR (T_max={NUM_EPOCHS})")
...
if __name__ == "__main__":
    ...
    # トレーニング再開のオプションを追加
    import argparse
    parser = argparse.ArgumentParser(description='Train Mahjong Transformer model')
    parser.add_argument('--resume', action='store_true', help='Resume training from the latest checkpoint')
    args = parser.parse_args()

    train_model(resume_training=args.resume)