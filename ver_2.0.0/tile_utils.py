...
def tile_id_to_index(tile: int) -> int:
    ...
    if not isinstance(tile, int) or not (0 <= tile <= 135):
        return -1
    # 赤ドラも通常の5として扱う
    if tile == 16: return 4   # 0m -> 5m (index 4)
    if tile == 52: return 13  # 0p -> 5p (index 13)
    if tile == 88: return 22  # 0s -> 5s (index 22)
...
def tile_index_to_id(index: int, prefer_red: bool = False) -> int:
    ...
    if index < 27: # Number tiles
        ...
        # 5の牌の非赤ドラIDは、例えば5mなら17, 18, 19のいずれか
        # ここでは代表として最初の非赤ドラID(17, 53, 89)を返す
...
def hand_ids_to_34_array(hand_ids: list[int]) -> list[int]:
    """
    136種の牌IDのリストを、34種の牌（0-33）をインデックスとする34要素の配列に変換します。
    mahjongライブラリの to_34_array の仕様に合わせます。
    
    Args:
        hand_ids (list[int]): 136種の牌IDのリスト。

    Returns:
        list[int]: 各牌の枚数を格納した34要素の配列。
    """
    # mahjongライブラリのユーティリティ関数をそのまま利用します。
    return TilesConverter.to_34_array(hand_ids)