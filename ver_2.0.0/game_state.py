...
# --- Dependency Imports ---
try:
    # Ensure these are in the same directory (ver_2.0.0) or Python path
    from tile_utils import tile_id_to_index, tile_id_to_string, tile_index_to_id
    from naki_utils import decode_naki
except ImportError as e:
    print(f"[FATAL ERROR in game_state.py] Cannot import from tile_utils/naki_utils: {e}")
    print("Ensure tile_utils.py and naki_utils.py are in the same directory or Python path.")
    # Attempting to continue with dummy functions if imports fail, but functionality will be broken.
    # It's better to fix the imports.
    def tile_id_to_index(tile: int) -> int: return tile // 4 if tile >= 0 else -1
    def tile_id_to_string(tile: int) -> str: return f"ERR_TILE_{tile}"
    def tile_index_to_id(index: int, prefer_red: bool = False) -> int: return index * 4 if index >= 0 else -1
    def decode_naki(m: int) -> dict: return {"type": "不明", "tiles": [], "from_who_relative": -1, "consumed": [], "raw_value": m}
    # sys.exit(1) # Consider exiting if imports fail critically
...
class GameState:
    ...
    def process_event(self, event_xml):
        """
        Generic event processor that dispatches to specific methods.
        This is useful for the data generation script.
        """
        tag = event_xml["tag"]
        ...
        for t_tag, p_id in self.TSUMO_TAGS.items():
            if tag.startswith(t_tag) and tag[1:].isdigit():
                self.process_tsumo(p_id, int(tag[1:]))
                return
        # Discard
        for d_tag, p_id in self.DISCARD_TAGS.items():
            if tag.startswith(d_tag) and tag[1:].isdigit():
                self.process_discard(p_id, int(tag[1:]), tag[0].islower())
                return
        # Naki
        if tag == "N":
            self.process_naki(int(attrib.get("who", -1)), int(attrib.get("m", 0)))
            return
        # Reach
        if tag == "REACH":
            self.process_reach(int(attrib.get("who", -1)), int(attrib.get("step", 0)))
            return
        # Dora
        if tag == "DORA":
            self.process_dora(int(attrib.get("hai", -1)))
            return
        # Agari
        if tag == "AGARI":
    ...
    def process_naki(self, naki_player_id: int, meld_code: int):
        ...
        decoded_tiles_structure = naki_info.get("tiles", []) # Use for hints
        from_who_relative = naki_info.get("from_who_relative", -1)

        if naki_type == "不明": print(f"[Warning] decode_naki failed for m={meld_code}. Skipping process_naki."); return
    ...
    def count_dora_in_hand(self, player_id: int) -> int:
        """Counts the number of dora tiles in a player's hand."""
        if not (0 <= player_id < NUM_PLAYERS):
            return 0
        dora_count = 0
        dora_indices = self.get_current_dora_indices()
        hand_indices = self.get_hand_indices(player_id)
        for d_idx in dora_indices:
            dora_count += hand_indices.count(d_idx)
        # 赤ドラのカウント
        red_fives_ids = [16, 52, 88] # 5m, 5p, 5s
        for red_id in red_fives_ids:
            if red_id in self.player_hands[player_id]:
                dora_count += 1
        return dora_count

    # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
    # ★★★★★★★★★  アクティベーション抽出用メソッド (ここから追加) ★★★★★★★★★
    # ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
    def predict_discard_with_model(self, model, player_id):
        """
        モデルを使って打牌予測を行い、予測結果と中間層のアクティベーションを返す。
        このメソッドは create_activation_dataset.py から呼び出されることを想定。
        """
        # 必要なモジュールを動的にインポート
        import torch
        import torch.nn.functional as F
        
        # predict.pyからモデルクラスをインポート（循環参照を避けるため関数内でインポート）
        # この関数が呼び出される文脈では、predict.pyはすでにロードされているはず
        from predict import MahjongTransformerV2WithAttention

        # 特徴量生成
        try:
            event_sequence = self.get_event_sequence_features()
            static_features = self.get_static_features(player_id)
        except Exception as e:
            print(f"[Error] Feature generation failed in predict_discard_with_model: {e}")
            return None, None, None, None, None

        # デバイスを取得（モデルが配置されているデバイスに合わせる）
        ...
        # フックが重複登録されないようにハンドルを管理
        hook_handle = None
        if hasattr(model, 'output_head') and len(model.output_head) > 0:
            hook_handle = model.output_head[0].register_forward_hook(get_activation('combined_vector'))
        else:
            print("[Warning] Model does not have 'output_head' or it's empty. Cannot attach hook for activation.")
            return None, None, None, None, None
        ...
        with torch.no_grad():
            try:
                if isinstance(model, MahjongTransformerV2WithAttention):
                ...
                # forward呼び出し後、フックによって activations_storage に値が格納される
                if 'combined_vector' in activations_storage:
                    activation_vector = activations_storage['combined_vector'].squeeze(0) # バッチ次元を削除
                
                probabilities = F.softmax(outputs, dim=1).squeeze(0).cpu().numpy()

            except Exception as e:
                print(f"[Error] Model forward pass failed: {e}", exc_info=True)
        ...
        # 最良の打牌を選択
        valid_discard_indices = self.get_valid_discard_options(player_id)
